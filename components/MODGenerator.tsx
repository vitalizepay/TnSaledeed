      ...
      <header className="bg-gradient-to-r from-indigo-50 to-violet-50 border-b border-slate-200/70 sticky top-0 z-40 w-full">
        <div className="w-full max-w-screen-2xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 sm:gap-3 text-sm flex-nowrap overflow-x-auto scrollbar-none">
            <button onClick={onOpenSaleDeed} className={headerBtnBlue} title="Go to Sale Deed">Sale Deed</button>
            <button onClick={onOpenSaleAgreement} className={headerBtnBlue} title="Go to Sale Agreement">Sale Agreement</button>
            ...
            <span className="hidden md:inline text-slate-300">|</span>
            <button onClick={logout} className="flex items-center gap-2 font-medium text-slate-700 hover:text-indigo-600 rounded-md px-3 py-1.5 whitespace-nowrap" title="Logout">
      ...
      <main className="w-full max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex-grow flex flex-col">
        <div className="rounded-2xl bg-gradient-to-r from-white/90 to-indigo-50/90 ring-1 ring-slate-200 shadow-md p-6 mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <HowItWorksStep icon={<UploadIcon className="h-6 w-6" />} title="1. Upload PDFs">Provide the sample MOD template, borrower Aadhaar and the property sale deed PDFs.</HowItWorksStep>
            <HowItWorksStep icon={<SparklesIcon className="h-6 w-6" />} title="2. System Drafts MOD">We extract borrower, loan and property particulars and fill your MOD in the template’s exact layout.</HowItWorksStep>
            <HowItWorksStep icon={<BigCheckIcon className="h-6 w-6" />} title="3. Review & Download">Preview the generated text, copy it, or export a DOCX to finalize and print.</HowItWorksStep>
          </div>
        </div>
        
        <div className="flex-grow flex flex-col lg:flex-row gap-8 h-full">
          <div className="lg:w-3/5">
            <div className="rounded-2xl bg-gradient-to-br from-indigo-50/80 via-white/80 to-violet-50/70 ring-1 ring-slate-200 shadow-lg p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <InputField id="bankName" label="Bank Name" value={bankName} onChange={(e) => setBankName(e.target.value)} placeholder="e.g., State Bank of India" icon={<BankIcon />} />
                <InputField id="loanAmt" label="Sanctioned Loan Amount" value={loanAmount} onChange={onLoanChange} placeholder="e.g., 25,00,000" icon={<RupeeIcon />} />
                <TextAreaField id="regOffice" label="Bank Registered Office" value={bankRegisteredOffice} onChange={(e) => setBankRegisteredOffice(e.target.value)} placeholder="Full registered office address" icon={<BankIcon />} />
                <TextAreaField id="branchAddr" label="Branch Address (Place of Deposit)" value={branchAddress} onChange={(e) => setBranchAddress(e.target.value)} placeholder="Full branch address" icon={<BankIcon />} />
              </div>
          ...
          {/* --- RIGHT COLUMN (Output) - Simplified for consistency and correct growth --- */}
          {(generatedText || isLoading) && (
            <div className="lg:w-2/5 flex flex-col gap-4">
              {isLoading ? (
                <div className="flex-grow flex items-center justify-center rounded-2xl bg-white/80 shadow-lg">
                  <Loader />
                </div>
              ) : (
                <>
